...
class ACEPotential(nn.Module):
    ...
    def __init__(self, num_radial: int, l_max: int, r_cut: float, hidden_dim: int = 32):
        super().__init__()

        self.descriptor = ACEDescriptor(num_radial, l_max, r_cut)

        # Input dimension: 2-body (num_radial) + 3-body (tp output)
        tp_out_dim = self.descriptor.tp.irreps_out.dim
        in_features = num_radial + tp_out_dim
    ...
    def forward(
        ...
        site_energies = self.mlp(descriptors)  # (num_nodes, 1)
